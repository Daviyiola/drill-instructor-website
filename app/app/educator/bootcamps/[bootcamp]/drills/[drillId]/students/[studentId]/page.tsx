...
export default async function Page({
  ...
  params: Promise<{bootcamp: string; drillId: string; studentId: string}>;
  searchParams: Promise<{attemptId?: string; review?: string}>;
}) {
  const [{bootcamp, drillId, studentId}, query] = await Promise.all([
    params,
    searchParams,
  ]);

  return <EducatorSubmissionDetail
    bootcamp={bootcamp.toLowerCase()}
    drillId={drillId}
