...
export default function EducatorBootcampOverview({bootcamp}: {bootcamp: string}) {
  ...
  const cards = [
    {href: "browse", eyebrow: "Question Library", title: "Browse questions", detail: "Review explanations and collect questions into a draft."},
  ...
  return <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8 lg:px-10 lg:py-10">
    ...
    <section className="relative mt-6 min-h-72 overflow-hidden rounded-[2.25rem] text-white shadow-soft">
      ...
      <div className="relative max-w-xl p-7 sm:p-10"><p className="text-xs uppercase tracking-[.24em] text-brand-gold">Educator bootcamp</p><h1 className="mt-3 text-5xl font-semibold">{bootcamp.toUpperCase()}</h1><p className="mt-4 text-sm leading-6 text-white/75">Manage assigned students, publish focused practice, and review performance.</p><Link href={`/app/educator/bootcamps/${bootcamp}/about`} className="mt-7 inline-flex min-h-12 items-center rounded-2xl bg-white px-5 text-sm font-medium text-brand-green">About {bootcamp.toUpperCase()}</Link></div>
    </section>
