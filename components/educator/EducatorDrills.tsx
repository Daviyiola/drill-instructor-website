...
import {formatSchoolDate} from "@/lib/dates/schoolTime";
import type {EducatorWorkspace} from "@/lib/types/educator";

function shortDate(value?: string, school?: EducatorWorkspace["school"]) {
  if (!value) return "No due date";
  return formatSchoolDate(value, school);
}
...
export default function EducatorDrills({bootcamp}: {bootcamp: string}) {
  ...
  function confirmReopen(row: EducatorDrillRow) {
    const nextWeek = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
    nextWeek.setSeconds(0, 0);
    setReopenDueAt(new Date(nextWeek.getTime() - nextWeek.getTimezoneOffset() * 60000).toISOString().slice(0, 16));
    setConfirm({row, action: "reopen"});
  }
  async function load() {
    if (!user) return;
    setError("");
  ...
  async function mutate() {
    ...
    if (action === "reopen" && (!reopenDueAt || !isFutureLocalDateTime(reopenDueAt))) {
      setBusy(""); setError("Choose a future due date before reopening the drill."); setConfirm({row, action}); return;
    }
    try {
      ...
      else await callFunction(user, "updateEducatorDrillStatusHttps", {bootcamp, drillId: row.drillId, action, ...(action === "reopen" ? {dueAt: new Date(reopenDueAt).toISOString()} : {})});
      await load();
    } catch (reason) { setError((reason as Error).message); }
    finally { setBusy(""); }
  }
  ...
  return <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8 lg:px-10">
    ...
    <div className="mt-6 flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs uppercase tracking-[.2em] text-brand-green/65">Assignment workshop</p><h1 className="mt-2 text-3xl font-semibold tracking-tight">Drills</h1><p className="mt-2 text-sm text-slate-600">Build question sets, assign students, and inspect submissions.</p></div><div className="flex flex-wrap gap-2"><Link href={`/app/educator/bootcamps/${bootcamp}/browse?returnTo=${encodeURIComponent(`/app/educator/bootcamps/${bootcamp}/drills`)}`} className="inline-flex min-h-12 items-center rounded-2xl border border-brand-green bg-white px-5 text-sm text-brand-green transition-colors hover:bg-brand-green/10">BROWSE QUESTIONS</Link><Link href={`/app/educator/bootcamps/${bootcamp}/drills/new`} className="inline-flex min-h-12 items-center rounded-2xl bg-brand-green px-5 text-sm text-white transition-colors hover:bg-brand-green/90">CREATE DRILL</Link></div></div>
    ...
    {!data ? <div className="mt-6 h-72 animate-pulse rounded-3xl bg-white" /> : rows.length ? <div className="mt-6 grid gap-4 xl:grid-cols-2">{rows.map((row) => {
      ...
      return <article key={row.drillId} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 flex flex-wrap gap-2">{row.status === "draft" ? <>
          ...
          <button onClick={() => duplicate(row)} className={secondaryActionClass}>Duplicate</button>
          <button onClick={() => row.status === "closed" ? confirmReopen(row) : setConfirm({row, action: "close"})} className={secondaryActionClass}>{row.status === "closed" ? "Reopen" : "Close"}</button>
    ...
    {confirm && <div className="fixed inset-0 z-[100] grid place-items-center bg-slate-950/45 p-4"><section className="w-full max-w-md rounded-[2rem] bg-white p-6 text-center"><h2 className="text-xl font-medium capitalize">{confirm.action} {confirm.row.title}?</h2><p className="mt-3 text-sm text-slate-600">{confirm.action === "delete" ? "This draft cannot be recovered." : confirm.action === "close" ? "Students cannot continue it, and submitted scores will be released." : confirm.action === "reopen" ? "Choose the new deadline for unfinished students." : confirm.action === "archive" ? "This closed drill will move out of your regular drill views." : "This drill will return to your Closed inbox."}</p>{confirm.action === "reopen" && <label className="mt-5 block text-left text-sm">New due date<input type="datetime-local" required min={minimumDueAt || undefined} value={reopenDueAt} onChange={(event) => setReopenDueAt(event.target.value)} className="mt-2 min-h-11 w-full rounded-xl border border-slate-200 px-3" /></label>}<div className="mt-6 grid grid-cols-2 gap-3"><button onClick={() => setConfirm(null)} className="min-h-11 rounded-xl border border-slate-300 transition-colors hover:bg-slate-50">CANCEL</button><button onClick={mutate} disabled={confirm.action === "reopen" && !isFutureLocalDateTime(reopenDueAt)} className="min-h-11 rounded-xl bg-brand-green text-white transition-colors hover:bg-brand-green/90 disabled:bg-slate-300">CONFIRM</button></div></section></div>}
  </div>;
}
